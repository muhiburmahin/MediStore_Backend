...
router.get("/verify-email/:token", validateRequest(AuthValidation.verifyEmailTokenValidation), AuthController.verifyEmail);
...
router.post("/change-password", authMiddleware(Role.CUSTOMER, Role.SELLER, Role.ADMIN), validateRequest(AuthValidation.changePasswordSchema), AuthController.changePassword);
router.post("/logout", validateRequest(AuthValidation.refreshTokenValidation), AuthController.logoutUser);
router.get("/login/google", validateRequest(AuthValidation.googleLoginQueryValidation), AuthController.googleLogin);
router.get("/google/success", validateRequest(AuthValidation.googleSuccessQueryValidation), AuthController.googleLoginSuccess);
router.get("/oauth/error", validateRequest(AuthValidation.oauthErrorQueryValidation), AuthController.handleOAuthError);
