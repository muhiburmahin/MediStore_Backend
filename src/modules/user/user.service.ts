...
const adminStats = async () => {
    try {
        const [
            totalUser, customerCount, sellerCount,
            totalCategories, totalMedicines, totalReviews, totalOrders
        ...
        const statusMap: Record<OrderStatus, { count: number; amount: number }> = {
        ...
        orderGroups.forEach((group) => {
            const statusKey = group.status?.toUpperCase() as OrderStatus;
...
const sellerStats = async (sellerId: string) => {
