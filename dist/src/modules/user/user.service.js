...
const adminStats = async () => {
    const [totalCount, customerCount, sellerCount, adminCount, totalCategories, totalMedicines, totalReviews,] = await prisma.$transaction([
        prisma.user.count(),
...
const customerStats = async (user) => {
    ...
    const orderAmountByStatus = Object.fromEntries(statuses.map((status) => [
        status,
        amountGroup.find((o) => o.status === status)?._sum.totalAmount ?? 0,
    ]));
    const orderCountByStatus = Object.fromEntries(statuses.map((status) => [
        status,
